...
# method types for plotting
method_types_supervision <- factor(
    rep(c("unsupervised", "unsupervised", "unsupervised", "semisupervised", "semisupervised"), 4), 
    levels = c("unsupervised", "semisupervised"))
...
for (i in seq_along(sample_names)) {
    ...
    p_ARI[[i]] <- 
        ggplot(d_plot_ARI, aes(x = method, y = ARI, 
                               color = interaction(method_types_spatial_coords, method_types_dimension_reduction), 
                               shape = method_types_SpatialDE)) + 
            facet_grid(~ method_types_supervision, scales = "free", space = "free_x") + 
            geom_point(stroke = 1.5) + 
            scale_shape_manual(values = c(4, 19)) + 
            scale_color_manual(values = brewer.pal(8, "Paired")[c(2, 6, 1, 5)]) + 
            ylim(c(-0.1, 0.5)) + 
