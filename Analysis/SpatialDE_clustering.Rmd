...
for (i in seq_along(sample_names)) {
    ...
    ### known marker genes (from Kristen; 81 genes)
    
    # run PCA on known marker genes (from Kristen; 81 genes)
    # note: remove NAs
    ...
    # run UMAP on known marker genes (from Kristen; 81 genes)
    set.seed(1234)
...
for (i in seq_along(sample_names)) {
    ...
    filename <- paste0("../plots/SpatialDE_clustering/ARI_sample_", 
                       gsub("^sample_", "", sample_names[i]), ".png")
    ggsave(filename, width = 6.5, height = 5.5)
}
```
