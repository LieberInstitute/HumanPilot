...
for (i in seq_along(sample_names)) {
    ...
    # remove PCA and UMAP from method names and levels (put in legend instead)
    levels_keep <- gsub("_UMAP", "", gsub("_PCA", "", levels(d_plot_sub$method)))[1:10]
    ...
    p_ARI[[i]] <- 
        ggplot(d_plot_ARI, aes(x = method, y = ARI, 
                               color = method_types_spatial_coords, 
                               shape = method_types_SpatialDE)) + 
            facet_grid(method_types_dimension_reduction ~ method_types_supervision, 
                       scales = "free", space = "free_x") + 
            geom_point(stroke = 1.5) + 
            scale_shape_manual(values = c(4, 19), 
                               labels = c("SpatialDE", "other methods")) + 
            scale_color_manual(values = brewer.pal(8, "Paired")[c(2, 6)], 
...
# remove PCA and UMAP from method names and levels (put in legend instead)
levels_keep <- gsub("_UMAP", "", gsub("_PCA", "", levels(d_plot_ARI_all$method)))[1:10]
...
ggsave(filename, width = 8, height = 6.5)
...
# remove PCA and UMAP from method names and levels (put in legend instead)
levels_keep <- gsub("_UMAP", "", gsub("_PCA", "", levels(d_plot_NMI_all$method)))[1:10]
...
ggsave("../plots/figure/fig_LMW_ground_truth.pdf", width = 3.8, height = 3)

panel_b_i <- 
    p_clustering_unsupervised_best + 
        ggtitle("Unsupervised clustering", subtitle = NULL) + 
        theme(legend.position = "none")
panel_b_ii <- 
    p_clustering_semisupervised_best + 
        ggtitle("Semisupervised clustering", subtitle = NULL) + 
        theme(legend.position = "none")
panel_b_iii <- 
    p_clustering_markers_best + 
        ggtitle("Clustering known markers", subtitle = NULL) + 
        guides(color = guide_legend(override.aes = list(color = colors)))
# using 'patchwork' package
panel_b_clustering <- panel_b_i | panel_b_ii | panel_b_iii
print(panel_b_clustering)
ggsave("../plots/figure/fig_LMW_clustering.pdf", width = 8.5, height = 3.5)
```
