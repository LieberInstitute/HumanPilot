...
colors <- rep(c(Polychrome::palette36.colors(7)[c(2:7, 1)], "black"), 2)
...
# calculate tests for significant differences in ARI (with vs. without spatial coords)
# 'with vs. without spatial coords', with blocking for (i) donor and (ii) spatially adjacent replicates
...
# experimental design
# note: 'location' refers to spatially adjacent replicates; factor 'location_nested' is nested within 'donor' 
# (for mixed effects model); alternatively factor 'location_fixed' has separate levels (fixed effects)
design <- tibble(
    sample_name = levels(res_ARI$sample_name), 
    donor = as.factor(paste0("donor_", rep(1:3, each = 4))), 
    location_nested = as.factor(paste0("location_", rep(c(1, 1, 2, 2), 3))), 
    location_fixed = as.factor(paste0("location_", rep(1:6, each = 2)))
)
...
# test for significant differences (with vs. without spatial coords)
...
for (m in seq_along(methods_pvals)) {
    ...
    # fit linear model (using fixed effects for adjacent spatial replicates)
    lm_tidy <- tidy(
        lm(ARI ~ method_types_spatial_coords + donor + location_fixed, data = d_test)
    )
    res_ARI_pvals[m] <- 
        lm_tidy %>% 
...
system("cp ../plots/SpatialDE_clustering/ARI_all_samples.pdf ../plots/figure/fig_LMW_Adjusted_Rand_Index.pdf")
```
...
ann_text <- tibble(
    method_types_supervision = factor(
        rep(c(rep("unsupervised", 3), "semisupervised", "markers"), 2), 
        levels = c("unsupervised", "semisupervised", "markers")), 
    method_types_dimension_reduction = factor(
...
p_ARI_annot <- 
    p_ARI_all + 
        geom_text(data = ann_text, 
                  aes(x = method, y = 0.45, shape = NULL, label = text), 
                  nudge_x = 0.5, 
                  color = "black", 
                  size = 4)
...
# save copy of final figure for manuscript
system("cp ../plots/SpatialDE_clustering/ARI_all_samples_pvals.pdf ../plots/figure/fig_LMW_Adjusted_Rand_Index_pvals.pdf")
```
...
# color palette
colors <- c(Polychrome::palette36.colors(7)[c(2:7, 1)], "black")
...
```{r, fig.width = 7.25, fig.height = 2.75}
# save panels for final figure (with custom formatting)
panel_a_ground_truth <- 
    p_ground_truth_combined + 
        facet_wrap(NULL) + 
        geom_point(size = 0.15) + 
        scale_color_manual(values = colors, labels = c(paste0("Layer ", 1:6), "White matter", "NA")) + 
...
ggsave("../plots/figure/fig_LMW_ground_truth.pdf", width = 3.75, height = 2.75)
...
panel_b_ii <- 
    p_clustering_semisupervised_best + 
        geom_point(size = 0.15) + 
        ggtitle("Semisupervised", subtitle = NULL) + 
        theme(legend.position = "none", 
              strip.background = element_blank(), 
              strip.text.x = element_blank())
panel_b_iii <- 
    p_clustering_markers_best + 
        geom_point(size = 0.15) + 
        ggtitle("Known markers", subtitle = NULL) + 
...
ggsave("../plots/figure/fig_LMW_clustering.pdf", width = 7.25, height = 2.75)
```
...
panel_a_ground_truth + 
    ...
    theme(title = element_blank(), 
          legend.position = "none")
ggsave("../plots/figure/color_palette/color_palette_Accent.pdf", width = 2.25, height = 2.5)

panel_a_ground_truth + 
    scale_color_manual(values = brewer.pal(8, "Set1"), labels = c(paste0("Layer ", 1:6), "White matter", "NA")) + 
    theme(title = element_blank(), 
          legend.position = "none")
ggsave("../plots/figure/color_palette/color_palette_Set1.pdf", width = 2.25, height = 2.5)
...
my_palette[6] <- "#FF8C00"  # dark orange
my_palette[5] <- "#FFD700"  # gold
my_palette[7] <- dark2[8]
my_palette[8] <- "#000000"  # black
...
panel_a_ground_truth + 
    ...
    theme(title = element_blank(), 
          legend.position = "none")
ggsave("../plots/figure/color_palette/palette_7colors.pdf", width = 2.25, height = 2.5)

panel_b_ii + 
    scale_color_manual(values = my_palette[c(4, 2, 3, 5, 8, 7, 6, 1)], labels = c(paste0("cluster", 1:8))) + 
    theme(title = element_blank(), 
          legend.position = "none")
ggsave("../plots/figure/color_palette/palette_8colors.pdf", width = 2.25, height = 2.5)
```
