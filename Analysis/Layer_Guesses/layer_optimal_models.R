...
levels(layer_guess_reordered_short) <-
    gsub('ayer', '', levels(layer_guess_reordered_short))
...
for (i in seq_len(nrow(marker_genes_summary))) {
    ...
    boxplot(
        ...
        main = paste(
            marker_genes_summary$gene_name[i],
            paste(ifelse(
                marker_genes_summary$species[i] == 'H', 'HM', 'MM'
            )),
            paste0(
                'p=',
                formatC(
...
pdf('pdf/markers_gene_optimal_scatter.pdf',
    width = 12,
    useDingbats = FALSE)
...
compute_GST <-
    function(type = 'f_stat_full',
        ranks.only = TRUE,
        orgs = c('mouse', 'human')) {
        set.seed(20200218)
        df <- subset(marker_genes_summary, species %in% orgs)
        geneSetTest(
...
apply_GST <- function(type) {
    data.frame(
        ...
        'mouse' = compute_GST(type, orgs = 'mouse'),
        'mouse_sim' = compute_GST(type, ranks.only = FALSE, orgs = 'mouse'),
        'human' = compute_GST(type, orgs = 'human'),
        'human_sim' = compute_GST(type, ranks.only = FALSE, orgs = 'human'),
        'type' = type,
        stringsAsFactors = FALSE
    )
...
dups <-
    marker_genes_summary$ensembl %in% marker_genes_summary$ensembl[duplicated(marker_genes_summary$ensembl)]
table(dups)
# FALSE  TRUE
#   114    12
...
fit_log <-
    glm(
        replicated ~ n_layers + species + list_km + list_bm + is_dup,
        family = binomial,
