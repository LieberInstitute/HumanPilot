...
}, t0_contrasts_match, fdrs0_contrasts_match,SIMPLIFY=FALSE)
...
pheatmap(t0_contrasts_TWAS_FDR,
					annotation_colors = ann_colors,
                    annotation_names_row = TRUE)
dev.off()
pdf("pdf/TWAS_Heatmaps_Bonf.pdf", h= 20,w=6)
pheatmap(t0_contrasts_TWAS_Bonf,
					annotation_colors = ann_colors,
                    annotation_names_row = TRUE)
dev.off()
...
lines(density(outGene2_match2$t[fdrs0_contrasts_match2$WM < 0.05]),col="red")
plot(density(outGene2_match2$t[fdrs0_contrasts_match2$Layer2 > 0.05]))
lines(density(outGene2_match2$t[fdrs0_contrasts_match2$Layer2 < 0.05]),col="red")
plot(density(outGene2_match2$t[fdrs0_contrasts_match2$Layer3 > 0.05]))
lines(density(outGene2_match2$t[fdrs0_contrasts_match2$Layer3 < 0.05]),col="red")

## DE
layer_de_enrich_tab = mapply(function(t,f) {
	layer = t > 0 & f < 0.1
	tt = table(DE_sig = outGene2_match2$adj.P.Val < 0.05, Layer=layer)
}, t0_contrasts_match2, fdrs0_contrasts_match2,SIMPLIFY=FALSE)

fisherList = lapply(layer_de_enrich_tab,fisher.test,alternative="greater")
bs2 = data.frame(P = sapply(fisherList, "[[", "p.value"), 
...
pheatmap(t0_contrasts_DE_FDR05,
					annotation_colors = ann_colors,
                    annotation_names_row = TRUE)
dev.off()
...
## BS1 #####
############
load("/users/ajaffe/Lieber/Projects/RNAseq/SzControl_DE_paper/rdas/DE_statistics_adjAndQsva.rda",verbose=TRUE)
outGene1 = outGene
...
}, t0_contrasts_match2a, fdrs0_contrasts_match2a,SIMPLIFY=FALSE)
...
fisherList_adj = lapply(layer_de_enrich_tab_adj,fisher.test,alternative="greater")
fisherList_qsva = lapply(layer_de_enrich_tab_qsva,fisher.test,alternative="greater")
fisherList_pca = lapply(layer_de_enrich_tab_pca,fisher.test,alternative="greater")

bs1_adj = data.frame(P = sapply(fisherList_adj, "[[", "p.value"), 
	OR=sapply(layer_de_enrich_tab_adj, getOR))
bs1_qsva = data.frame(P = sapply(fisherList_qsva, "[[", "p.value"), 
	OR=sapply(layer_de_enrich_tab_qsva, getOR))
bs1_pca = data.frame(P = sapply(fisherList_pca, "[[", "p.value"), 
...
}, t0_contrasts_match2b, fdrs0_contrasts_match2b,SIMPLIFY=FALSE)

fisherList_hg38 = lapply(layer_de_enrich_tab_hg38,fisher.test,alternative="greater")
bs1_hg38 = data.frame(P = sapply(fisherList_hg38, "[[", "p.value"), 
...
}, t0_contrasts_match3, fdrs0_contrasts_match3,SIMPLIFY=FALSE)
fisherList_sczd = lapply(layer_sczd_enrich_tab,fisher.test,alternative="greater")
pe_sczd = data.frame(P = sapply(fisherList_sczd, "[[", "p.value"), 
...
fisherList_asd = lapply(layer_asd_enrich_tab,fisher.test,alternative="greater")
pe_asd = data.frame(P = sapply(fisherList_asd, "[[", "p.value"), 
...
fisherList_bpd = lapply(layer_bpd_enrich_tab,fisher.test,alternative="greater")
pe_bpd = data.frame(P = sapply(fisherList_bpd, "[[", "p.value"), 
...
de_enrich_list = list(BS2_SCZD = bs2, BS1_SCZD = bs1_qsva, 
	BS1_SCZD_remap = bs1_hg38,
	PE_SCZD = pe_sczd, PE_ASD = pe_asd, PE_BD = pe_bpd)
...
pdf("publicData/LIBDdlpfc_Stage_markerList_panelPlot.pdf",h=20,w=13)
...
for(i in seq(along=gIndexes)) {
	...
	for(j in 1:nrow(geneTab)) {
		if(j %in% legIndex) par(mar=c(1,4,3,0)) else par(mar=c(1,2,3,2))
		layer = c(geneTab[j,"1"], rep(geneTab[j,"2"],3), rep(geneTab[j,"3"],2),
			rep(geneTab[j,"4"],3), rep(geneTab[j,"5a"],3), 
			rep(geneTab[j,"5b"],3), rep(geneTab[j,"6"],8), geneTab[j,"SP"])
		layer = layer*yy[j] # weight by expression
		layer[layer > 10] = 10
		layerMat = matrix(rev(layer), nc = 1)
		image(t(layerMat), col = cols, zlim = c(0,10),axes=FALSE,
			main = geneTab$MouseSym[j],cex.main=1.6)
		if(j %in% legIndex) axis(2, at = c(0.5,5, 10.5, 13.5, 16.5, 20, 23.75)/24,
			labels = c("SP", "VI", "Vb", "Va", "IV", "II/III", "I"),cex.axis=2,las=2)
		if(j == 1) text(x=0.5,y=0.5, names(gIndexes)[i],srt=90, cex=2.5, offset=0)
	}
}
dev.off()
