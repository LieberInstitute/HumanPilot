...
for (i in seq_along(sample_names)) {
    ...
    # apply filtering
    sce_filt <- sce_sub_SpatialDE[(!ix_low_UMI) & (!ix_mito), ]
    dim(sce_filt)
...
list_overlap <- list(
    ...
    SpatialDE_filtered = genes_SpatialDE_filt_pooled$gene_id, 
    HVGs = genes_HVGs$gene_id, 
    pseudobulk_layers = genes_pseudobulk$gene_id
)

#pdf("../plots/SpatialDE_genes_analysis/upset_genes_SpatialDE_SpatialDEfilt_HVGs_pseudobulk.pdf", width = 6, height = 6)
upset(
...
#pdf("../plots/SpatialDE_genes_analysis/upset_genes_SpatialDEfilt_HVGs_pseudobulk.pdf", width = 6, height = 6)
upset(
    fromList(list_overlap), 
    nsets = length(list_overlap), 
    order.by = "degree", 
